...
def compose_parent_pk_kwarg_name(value):
    return "{0}{1}".format("parent_lookup_", value)
...
class ExtendedActionLinkRouterMixin(object):
    ...
    _routs = routes[2:4] + routes[:2]  # first routes should be dynamic (because of urlpatterns
                                       # position matters) left self.routs for backward
...
class DefaultRouter(ExtendedRouterMixin, DefaultRouter):
    pass
